...
namespace NodeCanvas.Framework{
	...
	abstract public partial class Node {
		...
		///Subscribe the node to a unity message send to the Graph Agent
		public void SubscribeToMessages(params string[] messages){ SubscribeToMessages(graphAgent, messages); }
		///Subscribe the node to a unity message send to the agent
		public void SubscribeToMessages(Component messageAgent, params string[] messages){
			if (messageAgent == null){
				Debug.LogError("Null Agent provided for Subscribe To Message");
				return;
			}
			var router = messageAgent.GetComponent<MessageRouter>();
			if (router == null)
				router = messageAgent.gameObject.AddComponent<MessageRouter>();
			for (var i = 0; i < messages.Length; i++)
				router.Listen(this, messages[i]);
		}

		///Unsubscribe the node from all messages send to the Graph Agent
		public void UnSubscribeFromAllMessages(){ UnSubscribeFromAllMessages(graphAgent); }
		///Unsubscribe the node from all messages send to the target agent
		public void UnSubscribeFromAllMessages(Component messageAgent){
			if (messageAgent == null)
				return;
			var router = messageAgent.GetComponent<MessageRouter>();
			if (router == null){
		...
		public void UnSubscribeFromMessages(params string[] messages){ UnSubscribeFromMessages(graphAgent, messages); }
		///Unsubscribe from a specific message to the target agent
		public void UnSubscribeFromMessages(Component messageAgent, params string[] messages){
			if (messageAgent == null)
				return;
			var router = messageAgent.GetComponent<MessageRouter>();
			if (router == null){
