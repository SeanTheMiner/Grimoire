		...
		virtual public void DrawNodeConnections(Vector2 canvasMousePos, float zoomFactor){
			...
			for (var connectionIndex = 0; connectionIndex < outConnections.Count; connectionIndex++){
				...
				if (connection != null){
					...
					if (e.button == 1 && e.type == EventType.MouseDown && sourcePortRect.Contains(e.mousePosition)){
					...
					if (e.button == 1 && e.type == EventType.MouseDown && targetPortRect.Contains(e.mousePosition)){
						graph.RemoveConnection(connection);
						e.Use();
						return;
					}
