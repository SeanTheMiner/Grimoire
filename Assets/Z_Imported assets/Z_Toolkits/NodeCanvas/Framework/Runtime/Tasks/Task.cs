...
namespace NodeCanvas.Framework{
	...
	abstract public partial class Task {
		...
		protected class EventReceiverAttribute : Attribute{
			...
			public bool ignoreOverrideAgent; //will always register the owner
			public EventReceiverAttribute(params string[] args){
				this.eventMessages = args;
			}
			public EventReceiverAttribute(bool ignoreOverrideAgent, params string[] args){
				this.ignoreOverrideAgent = ignoreOverrideAgent;
				this.eventMessages = args;
			}
		}
...
//			}
		...
		bool Initialize(Component newAgent, Type newType){
			...
			if (msgAttribute != null){
				SubscribeToAgentEvents(msgAttribute.ignoreOverrideAgent? ownerAgent : newAgent, msgAttribute.eventMessages);
			}
		...
		//Gather errors for user convernience, used basicaly in the editor.
			...
			foreach (var field in this.GetType().RTGetFields()){
				...
				if (field.RTGetAttribute<RequiredFieldAttribute>(true) != null){
					...
					if (typeof(BBParameter).RTIsAssignableFrom(field.FieldType)){
						var bbParam = field.GetValue(this) as BBParameter;
						if (bbParam == null){
							errors.Add("Null BBParameter Object!");
