...
namespace NodeCanvas.Framework{
	...
	abstract public partial class Graph : ScriptableObject, ITaskSystem, ISerializationCallbackReceiver, IScriptableComponent {
		...
		public IBlackboard blackboard{
			...
			{
				#if UNITY_EDITOR
				//TODO: Fix properly (This is done for when the blackboard is a component and has been removed by the user)
				if (_blackboard is Object && _blackboard as Object == null){
					return null;
				}
				#endif

				return useLocalBlackboard? localBlackboard : _blackboard;
			}
			set
			{
				if (isRunning && _blackboard != value){
					Debug.LogWarning("Setting the graph blackboard while the graph is running is not possible");
					return;
				}

				if (useLocalBlackboard && value != localBlackboard){
					return;
				}
		...
		public void StartGraph(Component agent, IBlackboard blackboard, System.Action<bool> callback = null){
			...
			MonoManager.AddUpdateMethod(OnGraphUpdate);
			OnGraphStarted();
			// place at the end?
		...
		public void Stop(bool success = true){
			...
			MonoManager.RemoveUpdateMethod(OnGraphUpdate);
			isRunning = false;
			isPaused = false;
		...
		public void SendEvent(EventData eventData){
			...
			//the agent gameobject will have MessageRouter component if a task has subscribed to an event. MessageRouter will hande and forward the event
		}
		...
		public static void SendGlobalEvent(EventData eventData){
			foreach(var graph in runningGraphs.ToArray())
